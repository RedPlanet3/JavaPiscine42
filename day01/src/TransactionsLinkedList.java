...
public class TransactionsLinkedList implements TransactionsList{
    ...
    @Override
    public void removeTransactionByIDAndCategory (UUID id, enTransferCategory category) throws TransactionNotFoundException
    {
        TransactionsNode list = listOfTransactions;
        TransactionsNode tmp;
        if (list == null)
        ...
        if (list.getData().getIdentifier().equals(id) && list.getData().getTransferCategory() == category) {
            listOfTransactions = listOfTransactions.getNext();
            this.size--;
            return;
        }
        if (tail.getData().getIdentifier().equals(id) && tail.getData().getTransferCategory() == category) {
            tail = tail.getPrew();
            tail.setNext(null);
            this.size--;
            return;
        ...
        while (list.getNext() != null)
        {
            if (list.getNext().getData().getIdentifier().equals(id) && list.getNext().getData().getTransferCategory() == category) {
                if (list.getNext().getNext() != null) {
                    tmp = list.getNext();
                    list.setNext(tmp.getNext());
                    tmp.setPrew(null);
